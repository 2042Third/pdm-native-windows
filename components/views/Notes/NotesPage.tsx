...
const NotesView = ({}) => {
  ...
  // const [headerValue, onChangeText] = React.useState(noteEditor.head); // original
  // const [noteValue, onChangeNote] = React.useState(noteEditor.content); // original
  const [headerValue, onChangeText] = React.useState("none");
  const [noteValue, onChangeNote] = React.useState("none");
  ...
  const shouldNotUpdateNote = () => {
    // return noteEditor.status === "fail" || noteEditor.note_id === ''; // orginal
    return false;
  }
  //
  ...
  return (
    <View style={[{flex:1},styles.notesBox, styles.container]}>
      ...
      <TextInput
        ...
        placeholder={"Unnamed note"} // Above is original
